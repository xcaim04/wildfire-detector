...
def test_predict_endpoint_success_flow(client):
    ...
    payload = {
        "X": 7, "Y": 5,
        "month": "aug", "day": "sat",
        "FFMC": 94.2, "DMC": 120.5, "DC": 602.4, "ISI": 15.8,
        "temp": 38.4, "RH": 18.0, "wind": 28.5, "rain": 0.0
    }
...
def test_predict_endpoint_invalid_calendar_strings(client):
    ...
    payload = {
        "X": 7, "Y": 5,
        "month": "invalid", "day": "sat",
        "FFMC": 90.0, "DMC": 40.0, "DC": 400.0, "ISI": 10.0,
        "temp": 30.0, "RH": 30.0, "wind": 15.0, "rain": 0.0
    }
    response = client.post("/wildfire/predict", json=payload)
